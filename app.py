...
def perform_ocr():
    ...
    Request:
        ...
        - prompt (optional): 自訂提示詞，預設為 "<image>\nFree OCR."
...
def perform_batch_ocr():
    ...
    接收多個圖片檔案，返回批次 OCR 辨識結果
    
    Request:
        - files: 多個圖片檔案（multipart/form-data）
        - prompt (optional): 自訂提示詞
        
    Returns:
        JSON 回應包含多個 OCR 文字結果
    """
    # 檢查是否有上傳檔案
...
if __name__ == '__main__':
    # 開發模式運行
    print("=" * 60)
    print("DeepSeek-OCR API 服務啟動中...")
    print("=" * 60)
    # 關閉 debug 模式以避免模型重複載入
    # 如需 debug，請使用 use_reloader=False
